...
If notifications are skipped unexpectedly, check `session.skipUnindexed`; the default skips Stop events whose `session_id` is absent from `~/.codex/session_index.jsonl`.
